...
如果 Windows 自带相机拍照更清晰，先确认它实际使用的拍照分辨率。当前排查到 Windows 相机使用过 `2560 x 1440`，而软件原来没有这个预设，所以 OpenCV 取流分辨率更低时会显得更糊。现在软件提供：

- `2560 x 1440` 预设。
- `Custom` 自定义分辨率，选择后手动输入 `Custom width` 和 `Custom height`。

注意：主界面实时预览为了降低 UI 压力，会把宽度大于 640 的图缩小显示。判断原始图是否清晰时，应点击 `View Left Original` / `View Right Original` 或直接查看保存的原图，不要只看主界面缩略预览。
